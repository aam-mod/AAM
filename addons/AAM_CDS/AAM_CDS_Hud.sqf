...
//VARIABLES
tpw_hud_version = "1.51";
tpw_hud_range = [5,300];	//_this select 0;
tpw_hud_colour = [0.55,0.56,0.91]; //_this select 1; // HUD
tpw_hud_friendlycolour = [0.55,0.56,0.91]; //_this select 2; // Friendlies
tpw_hud_enemycolour = [1,0,0]; //_this select 3; // Enemies
tpw_hud_civcolour = [0.55,0.56,0.91]; //_this select 4; // Civs
tpw_hud_squadcolour = [0.55,0.56,0.91]; //_this select 5; // Squad and markers
tpw_hud_alpha = 1; //_this select 6; // initial HUD brightness
AAM_cds_no = 0; // for disabeling HUD elements

tpw_hud_asl = [1,0.97,0.01,1]; //_this select 7; // ASL [1,0.58,0.46,1]
tpw_hud_azt = [1,0.47,0.01,1]; //_this select 8; // AZT
tpw_hud_grd = [1,0.01,0.01,1]; //_this select 9; // GRD
tpw_hud_lmt = [1,0.93,0.96,1]; //_this select 10; // LMT [1,0.34,0.56,1]
tpw_hud_hlt = [1,0.01,0.90,2]; //_this select 12; // HLT [1,0.39,0.5,1]

tpw_hud_rng = [0,0.5,0.55,1]; //_this select 13; // RNG
tpw_hud_vel = [0,0.4,0.55,1]; //_this select 14; // VEL
tpw_hud_tmp =[0,0.5,0.5,1]; //_this select 11; // TMP
tpw_hud_prx = [0,0.5,0.55,1]; //_this select 15; // PRX
tpw_hud_unit = [0,1,0.25,0.75]; //_this select 16; // UNITS
tpw_hud_offset = [0,0]; //_this select 17; // Offset[-0.01,0.29]
tpw_hud_scale = 1; //_this select 18; // Scale
tpw_hud_textscale = 1; //_this select 19; // Text scale
tpw_hud_degradation = 1; //_this select 20; // HUD degradation
tpw_hud_thirdperson = 0; //_this select 21; // No HUD in 3rd person view
tpw_hud_addtac = 0; //_this select 22; // Add TAC glasses
tpw_hud_icons = [4,28,10,21,22,21,28,21,34,21]; //_this select 23; // Icon types
tpw_hud_vehiclefactor = 1; //_this select 24; // Range factor for vehicles
tpw_hud_audible = 0; //_this select 25; // Range factor for vehicles

// DEFAULT HUD ELEMENT LAYOUT (SCRIPT VERSION)
	tpw_hud_asl_txt = "%1<t size='0.5' font='AAMFS'><br /> ASL</t>";
	tpw_hud_azt_txt = "<t size='0.8' font='AAMFS'>%1<t size='0.5' font='AAMFS'><br />   AZT<br />    %2</t>";
	tpw_hud_grd_txt = "%1<t size='0.5' font='AAMFS'><br />         GRD</t>";
	tpw_hud_lmt_txt = "%1%2<t size='0.5' font='AAMFS'><br />LMT</t>";
	tpw_hud_tmp_txt = "%1<t size='0.5' font='AAMFS'><br />TMP</t>";/////////////
	tpw_hud_hlt_txt = "%1<t size='0.4' font='AAMFS'><br />    BPM</t>"; //tpw_hud_hlt_txt = "%2<t size='0.5' font='AAMFS'><br />BPM %1</t>";
	tpw_hud_rng_txt = "%1<t size='0.5' font='AAMFS'><br />RNG</t>";/////////////
	tpw_hud_vel_txt = "%1<t size='0.5' font='AAMFS'><br />VEL</t>";/////////////
	tpw_hud_airvel_txt = "%1<t size='0.5' font='AAMFS'><br />VEL KTS</t>";
...
tpw_hud_fnc_displayparams =
	...
	for "_i" from 0 to (count tpw_hud_params - 1) do
		...
		_ctrl = _display displayctrl (100001 + _i);//100001
		...
		_ypos = _ypos * safezoneH + safezoneY; //safezoneW + safezoneX;
...
tpw_hud_fnc_goggles =
	...
	if (((goggles _unit) in["G_Tactical_Black","G_Tactical_Clear","G_Goggles_VR","ADF_Wrap_CDS","ADF_SOTGMask_Standard","ADF_SOTGMask_black_Standard"]) || ((headgear _unit) in ["H_HelmetSpecO_blk","H_HelmetSpecO_ocamo","H_HelmetLeaderO_oucamo","H_HelmetLeaderO_ocamo"])) then {_unit setvariable ["tpw_hud_goggles",1];} else {_unit setvariable ["tpw_hud_goggles",0];};
	};
		...
		if (!isnil "_unit" && {!isnull _unit}) then
			...
			switch _status do
				...
				case "visible":
					...
					_unit setvariable ["tpw_hud_unitcolour",(_colour + [aam_cds_no])];//[tpw_hud_alpha])];
					_visible_temp set [count _visible_temp, _unit]; // move to visible units array
					};
				case "unsure":
					{
					_unit setvariable ["tpw_hud_unitcolour",(_colour + [aam_cds_no])];//[tpw_hud_alpha*0.5])];
...
tpw_hud_fnc_unitshow =
	...
	["tpw_hud","oneachframe",
		...
		for "_ctj" from 0 to (count tpw_hud_markerarray - 1) do
			...
			drawIcon3D [tpw_hud_markericon,(tpw_hud_squadcolour + [tpw_hud_alpha]),_ipos,_scale,_scale,0,_dist,0,tpw_hud_unittextsize, "AAMFS","left",true];
			};
